...
// WebGL rendering and playback is handled by useShaderController (inside useEditorState)
...
/** Location state passed when navigating from response scorer */
interface EditorLocationState {
  initialCode?: string;
  initialChatMessages?: ChatMessageNode[];
}
...
function EditorPage() {
  ...
  // Extract initial state from navigation (e.g., from response scorer)
  const locationState = location.state as EditorLocationState | null;
  const initialCode = locationState?.initialCode;
  const initialChatMessages = locationState?.initialChatMessages;

  // UI state
  const [isAIPanelOpen, setIsAIPanelOpen] = useState(!!initialChatMessages?.length);
  ...
  // Initialize editor state with custom hook (includes shader controller)
  const editorState = useEditorState({ slug, initialCode });

  // Get controller from editorState for WebGL/playback access
  const { controller } = editorState;
      ...
      {isMobile ? (
        ...
        <div className="flex-1 flex overflow-hidden">
          <ResizablePanelGroup direction="horizontal" className="flex-1" onLayout={controller.handlePanelResize}>
            ...
            <ResizablePanel defaultSize={40} minSize={controller.leftPanelMinSize}>
